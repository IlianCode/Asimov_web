...
const getNotes_Eleve = async (idEleve, Id_Eleve) => {
    return new Promise((resolve, reject) => {
        let sql='SELECT idMatiere, Matiere, Nom, note, date, titre, Moyenne FROM ( ' +
                 'SELECT m.idMatiere AS idMatiere, m.nom AS Matiere, CONCAT(e.nom, " ", e.prenom) AS Nom, null AS note, null AS date, null AS titre, Round(avg(n.note),2) AS Moyenne FROM notes n ' +
                 ...
                 'SELECT m.idMatiere AS idMatiere, m.nom AS Matiere, CONCAT(e.nom, " ", e.prenom) AS Nom, n.note AS note, n.dateNote AS date, n.Titre AS titre, null AS Moyenne FROM notes n ' +
                 'INNER JOIN matiere m ON m.idMatiere = n.Id_Matiere ' +
                 'INNER JOIN eleve e ON e.idEleve = n.Id_Eleve AND e.idEleve = ? ' +
                 ') AS OK;';
...
/* SELECT idMatiere, Matiere, Nom, note, date, titre, Moyenne FROM ( 
SELECT m.idMatiere AS idMatiere, m.nom AS Matiere, CONCAT(e.nom, " ", e.prenom) AS Nom, null AS note, null AS date, null AS titre, Round(avg(n.note),2) AS Moyenne FROM notes n
INNER JOIN matiere m ON m.idMatiere = n.Id_Matiere
INNER JOIN eleve e ON e.idEleve = n.Id_Eleve AND e.idEleve = 2
GROUP BY Matiere, Nom
